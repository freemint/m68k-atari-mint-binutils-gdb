...
"warning: traditional format not supported for relocatable output");
		...
		{
			_bfd_error_handler ("warning: traditional format is obsolete");
			_bfd_error_handler (
...
MY_bfd_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
	...
	if (ibfd->xvec == &(MY (vec)) && obfd->xvec == &(MY (vec))) 
	{
		...
		if (myinfo_in->tparel != NULL) 
		{
			...
			if (bfd_bread (myinfo_in->tparel, 1 * myinfo_in->tparel_size, ibfd) 
...
#define _MINT_F_SHTEXT 0x800
	...
	{
		/* Umh, don't know why... */
		relocation += (-32768 - (obj_textsec (output_bfd))->_raw_size -228);
	}
		   ...
		   struct external_exec *exec_bytes)
{
	...
	if (myinfo->symtab_size == 0)
		...
		myinfo->tparel_pos = N_SYMOFF (*execp) + myinfo->symtab_size;
	...
	PUT_WORD (abfd, execp->a_text + EXEC_BYTES_SIZE - 28, exec_bytes->g_text);
...
link_write_traditional_syms (bfd *abfd, struct bfd_link_info *info)
{
	...
	for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
	{
		...
		/* First write out a symbol for the archive if we do not
		   ...
		   one.  */
		if (ibfd->my_archive != last_archive && ibfd->my_archive != NULL) 
		{
		...
		&& strip != strip_all
		&& (strip != strip_some
		  || bfd_hash_lookup (info->keep_hash, ibfd->my_archive->filename,
				  FALSE, FALSE) != NULL)
		&& discard != discard_all) 
		{
			...
			written_bytes = write_dri_symbol (abfd, ibfd->my_archive->filename,
				A_TFILE, val);
			...
			if (pass) {
				...
				switch (strip)
				{
					...
					case strip_debugger:
						break;
					case strip_some:
						if (bfd_hash_lookup (info->keep_hash, name,
							FALSE, FALSE) == NULL)
							skip = TRUE;
						break;
					case strip_all:
