...
static int squirt_out_tparel (bfd*, struct internal_exec*,
				      struct external_exec*);
...
/* This macro overrides the one found in libaout.h */
...
m68kmint_prg_final_link_relocate (reloc_howto_type* howto, bfd* input_bfd, asection* input_section,
				  bfd_byte* contents, bfd_vma address, bfd_vma value, bfd_vma addend, struct reloc_std_external* rel)
{
  bfd_vma relocation;
...
#define _MINT_F_SHTEXT 0x800
  ...
  if (howto->pc_relative)
    ...
    {
      /* Umh, don't know why... */
      relocation += (-32768 - (obj_textsec (output_bfd))->size
		     -228);
    }
...
static int
squirt_out_tparel (bfd* abfd, struct internal_exec* execp, struct external_exec* exec_bytes)
{
  struct mint_internal_info* myinfo = obj_aout_ext (abfd);
