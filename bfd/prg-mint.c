/* BFD backend for traditional MiNT executables.
   ...
   Written by Guido Flohr (guido (somewhere at) imperia.net).
...
*/
	...
	bfd_byte e_trsize[4];		/* Length of text relocation info.  */
	bfd_byte e_drsize[4];		/* Length of data relocation info.  */
...
static bfd_boolean MY_bfd_free_cached_info PARAMS ((bfd*));
static bfd_boolean MY_bfd_copy_private_bfd_data PARAMS ((bfd*, bfd*));
...
{										\
	...
	if (bfd_bwrite ((PTR) &exec_bytes, 1 * EXEC_BYTES_SIZE, abfd)		\
	   != EXEC_BYTES_SIZE)							\
		return FALSE;							\
...
MY_object_p (bfd *abfd)
{
	...
	if (bfd_bread ((PTR) &exec_bytes, 1 * EXEC_BYTES_SIZE, abfd)
	   != EXEC_BYTES_SIZE)
	{
	...
	if (exec_bytes.g_branch[0] == 0xde && exec_bytes.g_branch[1] == 0xad)
	{
	...
	else 
	{
		...
		if (bfd_seek (abfd, myinfo->tparel_pos, SEEK_SET) != 0
		   || (bfd_bread (myinfo->tparel, 1 * myinfo->tparel_size, abfd)
		      != myinfo->tparel_size))
		{
			return 0;
		}
...
static int vma_cmp (const void *v1, const void *v2)
{
...
(Executables in traditional format will not be recognized");
	...
	if (NAME(aout,final_link) (abfd, info, MY_final_link_callback) != TRUE)
		return FALSE;
...
MY_bfd_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
	...
	if (ibfd->xvec == &(MY (vec)) && obfd->xvec == &(MY (vec))) 
	{
		...
		{
			if (bfd_seek (ibfd, myinfo_in->tparel_pos, SEEK_SET)
			   != 0)
				return FALSE;

			if (bfd_bread (myinfo_in->tparel,
			   1 * myinfo_in->tparel_size, ibfd) 
...
static const char*
m68kmint_prg_find_symbol_name (reloc_howto_type *howto,
			       bfd *input_bfd,
			       bfd_byte *location,
			       struct reloc_std_external *rel)
{
...
#define _MINT_F_SHTEXT 0x800
		...
		if (input_section == obj_textsec (input_bfd))
		{
			if (!r_extern)
			{
				...
				if (r_index != N_TEXT && r_index != (N_TEXT | N_EXT))
					...
					+ input_section->output_section->rawsize))
			{
				error_found = TRUE;
			}
			else if (relocation == (input_section->output_section->vma
	...
	{
		/* Umh, don't know why... */
			relocation += (-32768 - (obj_textsec (output_bfd))->rawsize
				-228);
	}
		   ...
		   struct external_exec *exec_bytes)
{
	...
	if (bfd_bwrite (myinfo->tparel, 1 * myinfo->tparel_size, abfd) 
	   != myinfo->tparel_size)
	return 1;
...
link_write_traditional_syms (bfd *abfd, struct bfd_link_info *info)
{
	...
	for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
	{
		...
		strip these symbols and if it differs from the last one.  */
		...
		if (write_archive_name
		    ...
		    || bfd_hash_lookup (info->keep_hash,
		       ibfd->my_archive->filename, FALSE, FALSE) != NULL)
		    && discard != discard_all) 
		{
			val = bfd_get_section_vma (abfd, obj_textsec (ibfd)->output_section)
				+ obj_textsec (ibfd)->output_offset;

			written_bytes = write_dri_symbol (abfd,
				ibfd->my_archive->filename, A_TFILE, val);
		...
		for (; sym < sym_end; sym++, sym_hash++)
		{
			...
			else
			{
				...
				if (symsec != (asection*) NULL)
					val = (symsec->output_section->vma + symsec->output_offset
...
write_dri_symbol (bfd *abfd, const char *name, bfd_vma type, bfd_vma value)
{
	...
	if (bfd_bwrite ((PTR) &sym, 1 * DRI_SYMBOL_SIZE, abfd) != DRI_SYMBOL_SIZE)
		return -1;
	...
	if (i > 0) 
	{
		...
		if (bfd_bwrite ((PTR) more_name, 1 * sizeof more_name, abfd)
		    != sizeof more_name)
			return -1;
